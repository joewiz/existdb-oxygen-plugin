...
- **Run Current Editor** (eXist-db menu / editor right-click / toolbar). Runs the whole active editor (or its selection) and opens the serialized result sequence in a **new editor tab**. This is the right tool for *running a script* and getting its full output, not for navigating result-by-result.
...
**Navigating from Builder results to the source node** (clicking a Results-view row to jump to the originating element) depends on eXist returning each result's source URI and position — tracked as [existdb-openapi #40](https://github.com/eXist-db/existdb-openapi/issues/40). Until that lands, Builder result rows show the serialized value but don't yet jump to source.
