...
**Validation engine.** Out of the box Oxygen validates XQuery with its bundled **Saxon** engine, which doesn't know eXist's modules — so `util:log(…)`, `xmldb:*`, and DB-resolved module imports get flagged as errors. The same **eXist-db (HTTP)** engine validates against eXist's *actual* compiler instead, so those validate clean. Two ways to use it:

- **Automatic for `exist://` resources** — anything opened from the database is compile-checked on open and after each edit, with problems listed in the **eXist-db** Problems view. No setup, nothing to select. (This runs *alongside* Oxygen's own inline validation; it doesn't replace the Saxon squiggle.)
- **As the default for *all* XQuery** — set **Preferences → XML → XSLT-XQuery → XQuery → Validation engine** to **eXist-db (HTTP)** (Global Options, or per-project under Project Options). Now Oxygen's inline, as-you-type validation uses eXist for **local files and `exist://` alike**. You can also pick the engine for a single file via its validation scenario if you only want it sometimes.

  Either way it validates against the editor's `exist://` server, or the default connection for a local file — so connect first. With no connection, the eXist engine reports nothing (rather than false errors), and Oxygen falls back to Saxon.
...
- **Read most built-in global options.** A plugin can only read the global settings whose keys are listed in `ro.sync.exml.options.APIAccessibleOptionTags` — a fixed whitelist of ~107 keys. `WSOptionsStorage.getOption(key, default)` silently returns the passed-in default for any non-whitelisted key, and `PluginWorkspace.getGlobalObjectProperty(key)` throws `IllegalArgumentException: Only keys in the APIAccessibleOptionTags interface are accessible via the API`. Notably **"Open last edited files from project"** (`open.last.edited.files`) is *not* on the whitelist, so a plugin cannot tell whether Oxygen will reopen the user's files on restart. We wanted our `exist://` editor-tab reopen to ride on that native setting; instead we ship our own **"Reopen eXist-db editor tabs on restart"** option (on by default) that mirrors it. *Unlocks:* honoring native settings directly instead of duplicating them as plugin options.
