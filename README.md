...
- **As the default for *all* XQuery** — the easiest way is the plugin's own toggle: open **Configure eXist-db Connections** (the eXist-db pane's gear) and check **"Use eXist-db (HTTP) as the default XQuery validation engine."** That sets Oxygen's global engine for you (and unchecking it restores the engine you had before). Equivalently, set it by hand at **Preferences → XML → XSLT-XQuery → XQuery → Validation engine** = **eXist-db (HTTP)** (Global Options, or per-project under Project Options). Either way, Oxygen's inline, as-you-type validation then uses eXist for **local files and `exist://` alike**. You can also pick the engine for a single file via its validation scenario if you only want it sometimes.

  Because the eXist engine validates over HTTP, it reports nothing while you're **disconnected** (a broken query won't be flagged) — so the toggle is best left on when you're working against a server, and off (back to Saxon) when offline.
